...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    var windowwidth = document.getElementById("randomPizzas").offsetWidth;// change to get element by id
...
function updatePositions() {
  ...
  //Moved the phase calculation into its own for loop that appends each phase to an array
  phaseList = [];
  for (var j = 0; j < 5; j++){
    phaseList.push(Math.sin((document.body.scrollTop / 1250) + (j % 5)));
  }
  //The pizza item styles are changed by accessing the relevant element of the phaseList array
